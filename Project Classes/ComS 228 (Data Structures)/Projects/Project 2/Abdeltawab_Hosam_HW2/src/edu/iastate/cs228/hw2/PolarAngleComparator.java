...
public class PolarAngleComparator implements Comparator<Point> {
	...
	public int compare(Point p1, Point p2) {
		int pol1 = this.comparePolarAngle(p1, referencePoint);
		int pol2 = this.comparePolarAngle(p2, referencePoint);
		if (p1.equals(p2)) {
			return 0;
		}
		if ((p1 != referencePoint && p2 != referencePoint && pol1 == pol2 && this.compareDistance(p1, p2) == -1)
				|| (p1 == referencePoint) || (p1 != referencePoint && p2 != referencePoint && pol1 < pol2)) {
			return -1;
		}
		return 1;
	}
	...
	public int comparePolarAngle(Point p1, Point p2) {
		int cProduct = crossProduct(p1, p2);
		if (cProduct == 0) {
			return 0;
		}
		if (p1.equals(referencePoint) || cProduct > 0) {
			return -1;
		}
	...
	public int compareDistance(Point p1, Point p2) {
		int dotProduct_1 = dotProduct(p1, p1);
		int dotProduct_2 = dotProduct(p2, p2);
		if (dotProduct_1 == dotProduct_2) {
			return 0;
		}
		if (dotProduct_1 < dotProduct_2) {
			return -1;
