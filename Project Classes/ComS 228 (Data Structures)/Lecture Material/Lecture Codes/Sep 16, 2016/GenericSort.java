 ...
 * by developing a sequence of six versions of the selection sort algorithm,
 * each one containing minor changes from the previous one.
 * 
 * Refer to the javadoc for java.lang.Comparable and java.util.Comparator.
 * You may also want to refer to summary document generics_notes.pdf posted
 * on WebCT.
 * 
...
public class GenericSort
{
  ...
  /**
   * 3. Same as above, but the caller can supply a custom Comparator in
   * order to sort strings differently than the "natural" or built-in
   * ordering provided in the String class.
   */
  ...
  /**
   * 4. Generic version of selection sort.  The type T is arbitrary, but
   * the caller must supply a Comparator.
   * 
   */
  ...
  /**
   * 5. Same as above, but imposes an upper bound on the type T.  Since T
   * is guaranteed to implement the Comparable interface, we can call the 
   * compareTo() method on objects of type T.  (When dealing with type
   * parameters, the keyword "extends" is used for both classes and interfaces.)
   */
  ...
  /**
   * 6. Alternate implementation of above which allows the re-use of 
   * version 4.  The natural ordering of type T is adapted to create
   * a Comparator for T with exactly the same ordering, and then
   * version 4 is called with that Comparator.
   */
  ...
  public static void main(String[] args)
  {
    ...
    Comparator<String> comp = new Comparator<String>()
    {
      ...
      {
        // returns a negative number if lhs is shorter, positive number if lhs is longer
    ...
    Vehicle[] cars = {new Vehicle("Studebaker", 1943),
                      new Vehicle("Chevy", 1958),
                      new Vehicle("Ford", 1957),
                      new Vehicle("Chevy", 1957),
                      new Vehicle("Ford", 1943)};
